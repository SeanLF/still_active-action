...
In `sbom` mode (below) the same precedence applies minus the two modes that need the native Ruby audit: `baseline` and `cyclonedx` are rejected up front, leaving SARIF and the `output-format` options.
...
- `sbom` does **not** support `baseline` or `cyclonedx`: both need the native audit's lockfile and snapshot, which a cross-ecosystem SBOM can't supply. Use `sarif`, or `output-format` `terminal`/`markdown`/`json`.
